<x-layout title="Partners">
    ...
    <div class="w-96">
        <h1 class="text-center font-bold mb-4 text-2xl">Partners</h1>
        <h1>{{ $individual->name }}</h1>
        <form action="{{ route('partners.store', $individual->id) }}" method="POST">
            @csrf
            @method('POST')
            ...
            <div class="mb-3">
                ...
                <input type="text" name="nama" id="nama" class="focus:border-none bg-transparent placeholder-gray-500 text-sm w-full border border-gray-300 rounded-lg px-2 py-2 mt-2" placeholder="Masukkan nama" required>
                @error('nama')
